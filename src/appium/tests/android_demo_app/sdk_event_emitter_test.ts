...
export default class SdkEventEmitterTest extends BaseTest {
  ...
  async execute(driver: WebdriverIO.Browser): Promise<TestResult> {
    ...
    try {
      ...
      const receivedSdkChangedEvent = "MainActivity: onNewMessage: OnSdkStatusChanged status ->";
      const receivedCampaignTriggerEvent = "MainActivity: onNewMessage: OnCampaignTriggered action:";
      const receivedSnapshotReportEvent = "MainActivity: onNewMessage: OnSnapshotReport"
      const receivedNotificationClickedevent = "MainActivity: onNewMessage: OnNotificationClicked campaign:"

      //console.log(`Microsegment report match: ${matchResult}`)

      if(!logs.includes(receivedSdkChangedEvent) || !logs.includes(receivedCampaignTriggerEvent) || !logs.includes(receivedSnapshotReportEvent) || !logs.includes(receivedNotificationClickedevent)){
        status = TestStatus.FAIL
        return {
          test: this.name,
          description: this.description,
          status: status,
          error: "Unable to find MainActivity received events prompts",
        };
      }
